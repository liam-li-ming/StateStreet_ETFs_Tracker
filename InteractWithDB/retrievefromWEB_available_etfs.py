...
class GetAvailableEtfs:
    ...
    def fetch_url_content(etf_list_url, wait_time = 5000):
        ...
        with sync_playwright() as p:
            ...
            # Wait for the ETF table rows to appear in the DOM rather than
            # sleeping a fixed amount of time. Falls back to wait_time (ms)
            # if the selector never appears (e.g. page structure changes).
            try:
                page.wait_for_selector('table tr td.fundTicker', timeout=10000)
            except Exception:
