...
class EquityEtfCompositionDb:
    ...
    def update_etf_info(self, etf_df):
        ...
        records = list(zip(
            etf_df['Ticker'], etf_df['Name'], etf_df['Domicile'],
            etf_df['Gross Expense Ratio'], etf_df['NAV'], etf_df['AUM'],
            [now] * len(etf_df)
        ))
    ...
    def insert_composition(self, composition_df, commit=True):
        """
        ...
        :param commit: If True (default), commit immediately. Pass False when batching
                       multiple ETFs in one transaction for better write performance.
        ...
        cursor = self.conn.cursor()
        records = list(zip(
            composition_df['etf_ticker'], composition_df['nav'],
            composition_df['shares_outstanding'], composition_df['total_net_assets'],
            composition_df['composition_date'], composition_df['component_name'],
            composition_df['ticker'], composition_df['identifier'],
            composition_df['sedol'], composition_df['weight'],
            composition_df['sector'], composition_df['shares'],
            composition_df['currency']
        ))
...
def fetch_and_store_all_etfs(db, asset_classes = None, skip_existing = True):
    ...
    # Single commit for all inserted compositions
    if success_count > 0:
        db.conn.commit()
